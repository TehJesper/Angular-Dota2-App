...
export class AppComponent {
  ...
  getMatchData() {
    this.http.get('https://api.opendota.com/api/matches/' + this.matchId).subscribe((matchData) => {
      ...
      this.heroesRadiant = [];
      this.victory = '';
      for (let i = 0; i < this.result.picks_bans.length && this.result.picks_bans[i].is_pick; i++) {

        if (this.result.picks_bans[i].team == 0) {
          this.heroesRadiant.push(this.heroes.find(x => x.id === this.result.picks_bans[i].hero_id).localized_name);
        }
        if (this.result.picks_bans[i].team == 1){
          this.heroesDire.push(this.heroes.find(x => x.id === this.result.picks_bans[i].hero_id).localized_name);
        }
      }
    })
  }
